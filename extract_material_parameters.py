...
def group_fitting_parameters_by_sample(popt_list,pvar_list,groups):
    ...
    samples_per_group = np.sum(mask,axis=1)
    ...
    if popt_list.ndim == 1:
        ...
        mean_popt = np.sum(popt_list,axis=1)/samples_per_group
        pstd_list = np.sqrt(np.sum(pvar_list,axis=1))/samples_per_group
    else:
        popt_list = mask[...,np.newaxis] * popt_list
        pvar_list = mask[...,np.newaxis] * pvar_list
        mean_popt = (np.sum(popt_list,axis=1).T/samples_per_group).T
        pstd_list = (np.sqrt(np.sum(pvar_list,axis=1)).T/samples_per_group).T
    return mean_popt,pstd_list
...
def calculate_fluid_properties(popt_list,pstd_list,rp,rc,pressure,gamma,gamma_std,mask=None):
    ...
    # Calculate STD of delta pressure assuming 5% error in pressure and following uncertainty propogation rules
    pressure_std = np.sqrt((.05*pressure)**2*np.ones_like(delta_pressure)+(gamma_std*2*(1/rp-1/rc))**2)
    ...
    mean_viscosity_std = np.sqrt(np.sum(viscosity_std**2))/len(viscosity)
...
def calculate_solid_properties(popt_list,pstd_list,rp,pressure,gamma,gamma_std,mask=None):
    ...
    delta_pressure = pressure - laplace_pressure
    # Calculate STD of delta pressure assuming 5% error in pressure and following uncertainty propogation rules
    pressure_std = np.sqrt((.05*pressure)**2*np.ones_like(delta_pressure)+(gamma_std*2*(1/rp))**2)
    ...
    mean_viscosity = np.mean(viscosity)
    mean_viscosity_std = np.sqrt(np.sum(viscosity_std**2))/len(viscosity)
    mean_elastic_modulus = np.mean(elastic_modulus)
    mean_elastic_modulus_std = np.sqrt(np.sum(elastic_modulus_std**2))/len(elastic_modulus)
...
if __name__ == "__main__":
    ...
    gc_laplace_experiment_file_list = ['laplace_pressure_gc/gc_'+str(i)+'.csv' for i in range(1,5)]
    ...
    print(f"GC Viscosity: {gc_viscosity:.2e} ± {gc_viscosity_std:.1e} Pa·s")
